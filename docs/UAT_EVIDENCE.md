...
| SCAN-001 | Private scanner deployed and connected to R2. | Run `pnpm uat:scan`; scan the generated harmless clean object. | Scanner returns provider-neutral `clean`; no file body, scanner key, R2 credentials or signed URLs are exposed. | Safe command output, timestamp, generated file ID/storage key prefix if needed. | PASS only with real Railway scanner result. | NOT RUN |
| SCAN-002 | Private scanner deployed and test-signature policy approved. | Run `pnpm uat:scan`; scan the generated UAT-only EICAR object. | Scanner returns provider-neutral infected/rejected status; normal storage lifecycle blocks signed download; object is cleaned up. | Safe command output, scan status/signature, cleanup confirmation. | PASS only when blocked after scan. | NOT RUN |
...
| BACKUP-001 | Neon production database selected and backup/PITR capability enabled. | Confirm Neon backup/PITR or restore point capability and retention for production branch/database. | Backup/restore point is available with timestamp and documented retention. | Neon backup/restore screenshot or provider evidence. | PASS only with Neon provider evidence. | NOT RUN |
| BACKUP-002 | Isolated Neon restore branch/database available. | Restore latest production backup/restore point into isolated branch/database and validate representative records. | Restore succeeds without damaging production; organisations, territories, users, provider connection metadata, published content, advertiser records and audit events validate. | Restore log, target branch/database, validation query results. | PASS only after rehearsal. | NOT RUN |
...
### SCAN-001 / SCAN-002 Live ClamAV Verification

Command: `pnpm uat:scan`

Date/time:

Environment: Neon UAT / Vercel preview / Cloudflare R2 private bucket / Railway ClamAV scanner

Result: NOT RUN

Evidence to capture:

- GREEN / AMBER / RED command output with no secret values, no signed URLs and no file bodies.
- Confirmation the harmless clean object scanned as `clean`.
- Confirmation the EICAR UAT-only object scanned as infected/rejected.
- Confirmation normal storage/scanning lifecycle blocks download for the infected EICAR object.
- Confirmation both temporary objects were deleted.

Notes:

- EICAR must only be generated by this UAT-gated command under `uat/verification/scan/`.
- Do not mark `SCAN-001` or `SCAN-002` PASS until this live command has been run against Railway.
