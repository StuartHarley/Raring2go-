...
## UAT-001E Cloudflare R2 Storage And ClamAV Scanning

Pilot file storage is Cloudflare R2 behind the existing provider-neutral `StorageProvider` boundary. Application/domain records store provider-neutral file references and storage keys; they must not expose R2 credentials, public object URLs or bucket internals to React components, public DTOs, audit events or general domain objects.

### Cloudflare R2 Requirements

- A private R2 bucket for pilot artefacts.
- R2 API credentials scoped to the pilot bucket.
- Public bucket access disabled.
- Signed upload/download URLs with short TTLs.
- Object keys generated by application services and validated as relative, normalised paths.
- Separate backup/retention policy recorded in UAT-004 before controlled pilot sign-off.
...
The R2 provider signs S3-compatible URLs server-side. The secret access key is never sent to the browser; upload/download requests receive only the time-limited URL and any required content headers.

### Download Safety

The storage boundary fails closed for downloads unless the file reference is:

- not deleted;
- marked `clean` by scanning; or
- explicitly marked `not_required` by an approved workflow.

Pending, failed or infected files must not be downloadable.

### ClamAV Scanner Boundary

Pilot scanning uses a private ClamAV-based service behind `FileScannerProvider`. The application integration is an HTTP adapter:
...
The scanner service is a deployment component, not a React or public-route concern. It should run privately, fetch or receive files from storage through controlled credentials, return `clean`, `infected` or `failed`, and optionally send signed scan-result callbacks. Do not mark production scanning GREEN until that scanner service is deployed and tested against real R2 objects.

### Controlled Storage Verification

1. Configure a private R2 bucket and scoped access keys.
2. Configure Vercel secrets for R2 and scanner values.
3. Create an upload intent for a pilot file and upload through the signed URL.
4. Confirm the object is not publicly accessible.
5. Run scanner integration and confirm clean files become downloadable.
6. Confirm pending, failed and infected files remain blocked.
7. Generate a signed download URL and confirm it expires.
8. Confirm R2 and scanner credentials do not appear in logs, audit, client HTML or returned DTOs.

UAT-001E is AMBER until real R2 and a private scanner service are configured and verified.
